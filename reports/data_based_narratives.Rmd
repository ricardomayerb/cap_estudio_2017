...
dext_to_gdp2 <- cs_deuda_externa %>% 
  filter(iso3c %in% coi) %>% 
  rename(deuda_ext_porc_pib = valor) %>% 
  filter(year <= params$end_year) %>% 
  filter(str_detect(indicador, "porcentaje")) 

narr_dext_to_gdp <- dext_to_gdp2 %>% 
  select(iso3c, year, deuda_ext_porc_pib) 
...
```{r  dext_avg_pctile_dygraph, echo=TRUE}
...
```{r secpub_debt_make_tables, eval=FALSE, include=FALSE}
## make the tables
...
```{r secpub_debt_show_k, eval=FALSE, include=FALSE, results="asis"}
## present the table
# kb_mr_3_desepub 
# kb_mr_3_gcentral
# # kb_mr_3_gsubn 
...
```{r secpub_debt_show_dt, eval=FALSE, include=FALSE, results="asis"}
dt_mr_3_desepub 
dt_mr_3_gcentral 
# dt_mr_3_gsubn 
dt_mr_3_spnf
...
```
```{r cv_make_tables, eval=FALSE, include=FALSE}
...
```{r cv_show_tables, eval=FALSE, include=FALSE}
# kb_mr_3_cv_anual 
dt_mr_3_cv_anual
```
...
```{r ci_show_tables, eval=FALSE, include=FALSE}
# kb_mr_3_ci_to_gdp 
dt_mr_3_ci_to_gdp 
```
...
```{r pb_make_tables, eval=FALSE, include=FALSE}
...
```{r pb_show_tables, eval=FALSE, include=FALSE}
# kb_mr_3_pb_to_gdp 
dt_mr_3_pb_to_gdp 
```
...
```{r exp_imp_make_tables, eval=FALSE, include=FALSE}
...
```{r exp_imp_show_tables, eval=FALSE, include=FALSE}
...
```{r shares_3_make_tables, eval=FALSE, include=FALSE}
...
```{r shares_3_show_tables, eval=FALSE, include=FALSE}
