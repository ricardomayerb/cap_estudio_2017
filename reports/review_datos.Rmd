...
```{r create_elements, message=FALSE, warning=FALSE}
...
Concentración de exportaciones: Herfindahl index, usando los 10 productos principales de exportacion por año
...
```{r cartera_vencida_qtr}
datatable(cartera_vencida_qtr %>% select(iso3c, year, quarter,
                                         cartera_vencida_percent)) %>% 
          formatRound('cartera_vencida_percent', 1)
```

Resultado fiscal

```{r sp_deuda}
datatable(sp_deuda_porc_pib %>% select(iso3c, year,cobertura, total_deuda_publica,
                                       deuda_externa_publica))
```

Crédito interno:

```{r}
datatable(tab_ci_qrt_gdp %>% select(iso2c, year, avg_spu_gdp, avg_spr_gdp, avg_tot_gdp)) %>% formatPercentage(c('avg_spu_gdp','avg_spr_gdp','avg_tot_gdp'), 1)
```
