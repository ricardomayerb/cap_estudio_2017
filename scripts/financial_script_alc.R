...
make_country_lists_by_quant <- function(df) {
  q_all = df %>% select(iso3c, ge n_group) %>% distinct(.keep_all = TRUE)
...
# pb_tot <- add_baselines(pb_tot, value_colname = "total_to_gdp_seas")
...
p_iso_list = purrr::map(coi_18, plot_ps_bk_fs)
