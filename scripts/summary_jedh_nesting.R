...
indicator_first = debt_data %>%
  group_by(group_ind_num, country) %>%
  summarise(nr_of_obs = n(),
            start = min(dateYQ),
            end = max(dateYQ)) %>%
  arrange(group_ind_num, nr_of_obs)


nest_by_ind <- debt_data %>% group_by(group_ind_num) %>% nest(.key=subdata) 

# 
# nest_by_ind2 <- nest_by_ind2 %>% 
#   mutate(model = purrr::map(data, .f = lm(lifeExp ~ year, data = .))
#   )
...
library(dplyr)
foo <- iris %>% nest(-Species)
moo <- chickwts %>% nest(weight)
